@testset "New API == Legacy API" begin
    ...
    @testset "ODE logistic equivalence" begin
        ...
        @test res_ode[1].best_aic == Kinbiont.AICc_evaluation2(
            length(p0_ode),
            2.0,
            res_ode[1].times,
            old_loss_ode;
    ...
    # -----------------------------------------------------------------
    @testset "AICc follows the configured fitting loss" begin
        p0 = [1.5, 0.3, 0.0]
        spec_nl = ModelSpec([MODEL_REGISTRY["NL_logistic"]], [p0])

        for loss_name in ("RE", "L2")
            Random.seed!(7)
            raw_nl = fit_NL_model(
                data_mat,
                "NL_logistic",
                ...
                NL_model_logistic,
                p0;
                type_of_loss = loss_name,
                smoothing    = false,
                maxiters     = 200,
            )
            old_loss  = Float64(raw_nl[2][end])
            old_times = Vector{Float64}(raw_nl[4])
            expected_aic = Kinbiont.AICc_evaluation2(
                length(p0),
                2.0,
                old_times,
                old_loss;
