...
function HPM_3_death_resistance_old(du, u, param, t)
    du[1] = -u[1] * param[2]
    du[2] =
        u[1] * param[2] + param[1] * u[2] * (1 - (u[3] + u[1] + u[2]) / param[6]) -
        param[3] * u[2]
...
function aHPM_3_death_resistance(du, u, param, t)
    ...
    du[3] =
        param[3] * u[2] +
        param[4] * u[3] * (1 - NaNMath.pow((u[3] + u[1] + u[2]) / param[5], param[6]))
end
...
function ODEs_aHPM_inhibition(du, u, param, t)
    ...
    du[2] =
        +u[1] * (param[2]) +
        param[4] * u[2] * (1 - NaNMath.pow((u[1] + u[2]) / param[3], param[5]))
end

function ODEs_HPM_SR(du, u, param, t)
    du[1] =
        -param[4] / (1 + param[3] * exp(-param[2] * t)) * u[1] + param[1] * u[1] -
        param[5] * u[1]
    du[2] = +param[5] * u[1]
end
...
function ODEs_adjusted_McKellar(du, u, param, t)
    ...
    du[2] =
        u[1] * (param[2]) +
        param[1] * u[2] * (1 - NaNMath.pow((u[1] + u[2]) / param[3], param[4]))
end
...
function ODE_Diauxic_replicator_2(du, u, param, t)
    if t <= param[3]
        ...
        du[1] =
            u[1] *
            (param[2] - u[1]) *
            (NaNMath.pow(u[1] - param[1], 2) * NaNMath.pow(u[1] - param[6], 2) + param[4])
    end
end
...
function ODE_gbsm_piecewise(du, u, param, t)
    if t <= param[4]
        du[1] =
            param[1] * (1 / (1 + NaNMath.pow(abs((t - param[4]) / param[2]), 2 * param[3])))
    else
        du[1] =
            param[1] * (1 / (1 + NaNMath.pow(abs((t - param[4]) / param[5]), 2 * param[6])))
    end
end
...
function hyper_gompertz(du, u, param, t)
    du[1] =
        param[1] * u[1] * NaNMath.pow(NaNMath.log(max(param[2], u[1]) / u[1]), 1 + param[3])
end
...
function hyper_logistic(du, u, param, t)
    du[1] =
        (param[1] / param[2]) *
        NaNMath.pow(u[1], (1 - param[3])) *
        NaNMath.pow(max(param[2], u[1] + 0.00001) - u[1], 1 + param[3])
end
...
function bertalanffy_richards(du, u, param, t)
    du[1] =
        NaNMath.pow(param[1] / param[2], param[3]) *
        u[1] *
        (NaNMath.pow(max(param[2], u[1] + 0.00001), param[3]) - NaNMath.pow(u[1], param[3]))
end
...
function ODE_von_bertalanffy(du, u, param, t)
    du[1] =
        u[1] *
        (param[1] * param[2] * t^(param[2] - 1) - param[3] * param[4] * t^(param[4] - 1))
end
...
function ODE_triple_piecewise_bertalanffy_richards(du, u, param, t)
    ...
    elseif t <= param[4]
        ...
        du[1] =
            param[5] *
            NaNMath.pow(u[1], param[6]) *
            (1 - NaNMath.pow(u[1] / param[7], param[6]))
    end
end
...
function baranyi_richards(du, u, param, t)
    du[1] =
        param[1] * (1 - (u[1] / param[2])) * (t^param[4]) /
        ((param[3])^(param[4]) + t^(param[4])) * u[1]
end
...
function baranyi_roberts(du, u, param, t)
    du[1] =
        param[1] *
        (1 - NaNMath.pow(u[1] / max(param[2], u[1] + 0.00001), param[5])) *
        (
            NaNMath.pow(t, param[4]) /
            (NaNMath.pow(param[3], param[4]) + NaNMath.pow(t, param[4]))
        ) *
        u[1]
end
