...
function fit_file_custom_ODE(
    ...
    for well_name in names_of_cols[2:end]
        ...
        temp_results_1 = fitting_one_well_custom_ODE(data, # dataset first row times second row OD
          string(well_name), # name of the well
          label_exp, #label of the experiment
          model, # ode model to use 
          lb_param, # lower bound param
    ...
    if write_res == true
        ...
        CSV.write(string(path_to_results, label_exp, "_parameters_custom_ODE_fitting.csv"), Tables.table(Matrix(parameter_of_optimization)))
...
function ODE_model_selection_file(
    ...
    for well_name in names_of_cols[2:end]
        ...
        temp_results_1 = ODE_Model_selection(data, # dataset first row times second row OD
          string(well_name), # name of the well
           label_exp, #label of the experiment
         ...
         calibration_OD_curve=calibration_OD_curve, #  the path to calibration curve to fix the data
          ...
          vectorized_temp_results =  expand_res_model_selection(temp_results_1[end],
         lb_param_array,
         string(well_name)
        )
        if verbose == true
    ...
    if write_res == true
        ...
        CSV.write(string(path_to_results, label_exp, "_parameters_model_selection.csv"), Tables.table(Matrix(parameter_of_optimization)))
...
function selection_ODE_fixed_change_points_file(
   ...
   PopulationSize = 300,
    ...
    for well_name in names_of_cols[2:end]
        ...
        temp_results_1 = selection_ODE_fixed_change_points(data, # dataset first row times second row OD
        string(well_name), # name of the well
        label_exp, #label of the experiment
        ...
        maxiters = maxiters,
         abstol = abstol
          ...
          vectorized_temp_results =  expand_res_segmentation(temp_results_1[1],
          lb_param_array,
          n_change_points + 1, 
          string(well_name),
          label_exp
    ...
    if write_res == true
        ...
        CSV.write(string(path_to_results, label_exp, "_parameters_model_cdp_nseg_",n_change_points + 1 ,".csv"), Tables.table(Matrix(parameter_of_optimization)))
